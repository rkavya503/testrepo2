...
from
(
SELECT event_uuid,startTime,issueTime,CAST(startTime AS CHAR) startTimeStr,estimatedEndTime,actualEndTime, eventlocations as eventlocation,IF(endtime IS NOT NULL, CAST(endTime AS CHAR), 'TBD') AS endtime , 
 CAST(MD5(CONCAT(product,CAST(startTime AS CHAR),IF(endtime IS NOT NULL, CAST(endTime AS CHAR),'TBD'))) AS CHAR)eventKey 
 FROM ( 
 SELECT product,event_uuid,e.startTime startTime,e.issuedTime issueTime,estimatedEndTime,actualEndTime, GROUP_CONCAT(event_detail.uuid) eventlocations 
 , 
