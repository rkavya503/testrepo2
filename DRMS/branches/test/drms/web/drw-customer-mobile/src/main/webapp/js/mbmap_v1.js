  ...
  var render = function (responseXML, doc) {
    ...
    doc.styles = styles;
      ...
      for (pm = 0; pm < placemarkNodes.length; pm++) {
		  ...
		  setTimeout(_renderPolygon(placemark,node,doc),1); 
      } // placemark loop
...
function _renderPolygon(placemark,node,doc){
       return function(){
             renderPolygon(placemark,node,doc);
       }
}
  
function renderPolygon(placemark,node,doc){
  // Init the placemark object
	
	placemark = {
	  name:  geoXML3.nodeValue(node.getElementsByTagName('name')[0]),
	  tag:  geoXML3.nodeValue(node.getElementsByTagName('tag')[0]),
	  description: geoXML3.nodeValue(node.getElementsByTagName('description')[0]),
  ...
  if (style) placemark.style = style;
	...
	var GeometryNodes = node.getElementsByTagName('MultiGeometry');
	...
	if (!!GeometryNodes && (GeometryNodes.length > 0)) {
	  for (var gn=0;gn<GeometryNodes.length;gn++) {
			 ...
			 var jsonObj2;  
			 try{
			 jsonObj2 = eval('(' + GeometryNodes[0].firstChild.nodeValue+')');
			 }catch(err){
			 jsonObj2 = eval('(' + GeometryNodes[0].textContent+')');
			 }
			 
			  for(var index=0; index<jsonObj2.length;index++){
				placemark.Polygon.push(jsonObj2[index].POLYGON[0]);
			  }
	}
  ...
  if (placemark.Point) {
	...
	if (placemark.Polygon) { // poly test 2
		...
		if (!!parserOptions.createPolygon) {
		 ...
		 // parserOptions.createPolygon(placemark, doc);//nothing but count
  ...
  var createMarker = function (placemark, doc) {
    ...
    if (!parserOptions.suppressInfoWindows) {
      var infoWindowOptions = geoXML3.combineOptions(parserOptions.infoWindowOptions, {
        content: '<div class="geoxml3_infowindow"><h3>' + placemark.tag + 
...
var createPolygon = function(placemark, doc) {
  ...
  for (var polygonPart=0;polygonPart<placemark.Polygon.length;polygonPart++) {
    ...
    //alert(placemark.Polygon[polygonPart].bounds);
     // var boundsTemp = placemark.Polygon[polygonPart].bounds;
     ...
     // bounds.union(boundsTemp);
    }
    var drawHoles = 0;
    if (!!parserOptions.drawingHoles) {
        drawHoles = parserOptions.drawingHoles;
    }   
    if(drawHoles == 1){
