...
<rich:panel styleClass="wizard-title-container">
	<span class="wizard-title">BIP - Update Active Events Estimated End Date&Time</span>
</rich:panel> 
		...
		<td>
			<h:panelGrid styleClass="filter-form-block" columns="2" id="reject-filter-block" style="width:100%">
				<h:panelGroup >
					...
					<h:outputText style="text-align: left;font-size:11px"   value="New Estimated End Date & Time:"/>	
					 <a4j:region>
                        <h:panelGroup>
			    ...
			    <input type="button" value="Clear Date" onclick="clearEstimatedDate();" style="margin-left:15px" title="Clear Date" class="wizard-btn" />
...
<rich:panel styleClass="wizard-button-container" style="border-top: 1px solid #DDDDDD"> 
	<a4j:commandButton value="Submit" styleClass="wizard-btn" style="margin-left:15px"
			action="#{activeEvents.doEdit}" title="Submit"	
			limitToList="true" reRender="event-creation-wizard"/>
	<a4j:commandButton value="Cancel" action="#{activeEvents.doCancel}" reRender="event-creation-wizard" immediate="true" title="Cancel" styleClass="wizard-btn"/>
	<a4j:commandButton value="Exit" action="#{activeEvents.doExit}" reRender="event-creation-wizard" immediate="true" title="Exit" styleClass="wizard-btn"/>
</rich:panel>
</h:form>