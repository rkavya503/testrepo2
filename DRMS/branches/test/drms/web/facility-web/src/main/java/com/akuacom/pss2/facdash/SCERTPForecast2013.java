...
import com.akuacom.pss2.program.scertp2013.SCERTPProgramEJB2013;
...
public class SCERTPForecast2013
{
	...
	public SCERTPForecast2013(String programName)
	{
        ...
        DlcManager manager =  EJBFactory.getBean(DlcManager.class);
        String clientName = FDUtils.getJSFClient().getName();
        ProgramParticipant pp = manager.findRtpStrategyByProgAndPartiForClient(programName, clientName, Boolean.TRUE);
        
        SCERTPProgramEJB2013 programEJB = 
			(SCERTPProgramEJB2013)systemManager.lookupProgramBean(programName);			
        ...
        Weather weather0 = weatherEAO.getWeatherByDate(yesterday.getTime());
        ...
        if(weather != null)
        {
        	forecastHighs[0] = weather0.getForecastHigh0();
			forecastHighs[1] = weather.getForecastHigh0();
			forecastHighs[2] = weather.getForecastHigh1();
			forecastHighs[3] = weather.getForecastHigh2();
			forecastHighs[4] = weather.getForecastHigh3();
			forecastHighs[5] = weather.getForecastHigh4();				
        }
...
//				DaySignals daySignals = getSignals(program, nowCalendar.getTime());
				...
				ParticipantRtpStrategy strategy = strategies.get(seasonStrings[i]);
				...
				for(int j = 0; j < TIME_BLOCKS; j++)
				{
					...
					hours[j].entries[i].setMode(getMode(strategy, j));
				}
	...
	private Mode getMode(ParticipantRtpStrategy strategy, int index)
	{
        ...
        Mode retVal = Mode.valueOf(value.toUpperCase());
