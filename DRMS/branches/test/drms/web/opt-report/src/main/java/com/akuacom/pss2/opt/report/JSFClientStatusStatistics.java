				...
				if( scchanged || isRangeChanged() || isSortColumnChanged() ){
					if(scchanged){
						if(sc.isSearchByParticipant()){
							...
							totalCount=getReportManager().getEventClientOfflineStatisticsCount(sc.getEvent());
						}else {
							totalCount = 0;
						}
					}
					if(sc.isSearchByParticipant()){
						...
						contents =getReportManager().findEventClientOfflineStatistics(
								sc.getEvent(),searchConstraint);
					}else{
