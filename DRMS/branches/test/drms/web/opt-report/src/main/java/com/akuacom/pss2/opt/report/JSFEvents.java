...
public class JSFEvents extends ReportTab {
	 ...
	 * @param report
	 */
	public JSFEvents(OperatorReports report) {
		super(report, OperatorReports.Tab.eventsTab,
...
//						searchConstraint=null;						
	...
	private AbstractTreeContentProvider<ReportEventPerformance> reportPerformanceTree = 
		...
		public List<ReportEventPerformance> getChildren(ReportEventPerformance parent) {
			...
			try {
				result = getReportManager().getChildrenPerformance(parent.getParticipantName(), 
						parent.getEventName(), 
	...
	private List<ReportEventPerformance> getReportEventParticipantPerformance(
		...
		contents = getReportManager().getEventParticipantPerformance(
				getSelectedEventName(), getSelectedProgramName(), searchConstraint);
		return contents;
	}
	private List<ReportEventPerformance> getReportAllEventPerformance()throws Exception{
		List<ReportEventPerformance> contents = Collections.emptyList();
		contents = getReportManager().getAllEventPerformance(
				getSelectedEventName(), getSelectedProgramName(), null);
		return contents;
	}
	/**
	...
	private List<ReportEvent> getReportEvent(SearchConstraint  searchConstraint , SearchCriteria sc)throws Exception{
		...
		if(sc.getParticipants()!=null){
			...
			contents = getReportManager().getReportEvent(sc.getEvent().getEventName(), searchConstraint);
		}
		return contents;
	}
	...
	private int getReportEventCount(SearchConstraint  searchConstraint , SearchCriteria sc) throws Exception{
		...
		if(sc.getParticipants()!=null){
			totalCount = getReportManager().getReportEventCount(
					...
					sc.getStartOfStartDate(), 
					sc.getEndOfEndDate());
		}else if(sc.getEvent()!=null&&(!sc.getEvent().getEventName().equalsIgnoreCase(""))){
			totalCount = getReportManager().getReportEventCount(sc.getEvent().getEventName());
		}
		return totalCount;
	}
