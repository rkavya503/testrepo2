...
public class JSFUsage extends ReportTab {
	...
	public JSFUsage(OperatorReports report) {
		super(report,OperatorReports.Tab.usageTab,
	...
	public void exportAction() throws Exception {
		if(!this.getSearchCriteria().isSearchByEvent()&&!this.getSearchCriteria().isSearchByParticipant())
			return;
	...
	// in this case(opt report).pass participantname or event name to flex, pass which one is depends on the search conditions, when search by participant passing paname other wise pass an event name
	// instead of retrive all related p in client, to-be get in server side
	public String getParticipants() {
		List<ParticipantVO> participants = this.getReport().getParticipant();
		List<String> participantNames = new ArrayList<String>();
		if(this.getSearchCriteria().isSearchByParticipant()){// if is searched by participant just return the selected participants 
			for(ParticipantVO vo: participants){
//				List<String> names = dataManager.findAggParticipantNames(vo.getParticipantName());
				participantNames.add(vo.getParticipantName());
			}
		}else{ // if is searched by event return none
	...
	public String getEventName() {
		if(this.getReport().getSearchCriteria().getEvent()==null){
			return "";
		}
		return this.getReport().getSearchCriteria().getEvent().getEventName();
	...
	private List<ReportSummaryVo> getUsageByDate() {
		...
		ReportEvent event = this.getSearchCriteria().getEvent();
		...
		List<ReportSummaryVo> temp = getUsageForEvent(event, dataManager);
		
		
		return convert4Display(temp);
	}

	private List<ReportSummaryVo> getUsageForEvent(ReportEvent event, HistoryReportManager dataManager) {
		// 1. get participants that participant in this event
		List<ParticipantVO> participants = this.getReport().getParticipant();
		Set<String> participantNames = new HashSet<String>();
		if(this.getSearchCriteria().isSearchByParticipant()){
			for(ParticipantVO vo: participants){
				List<String> names = dataManager.findAggParticipantNames(vo.getParticipantName());
				participantNames.addAll(names);
			}
			...
			if(event!=null){
				List<ReportEventParticipant> parts = new ArrayList<ReportEventParticipant>();
				for(String name: participantNames){
					ReportEventParticipant ep = new ReportEventParticipant();
					ep.setParticipantName(name);
					parts.add(ep);
				}
		...
		if(this.getSearchCriteria().isSearchByParticipant()&&participantNames.size()>1){
			setAggregator(true);
			// set all participants names
			setAllPaticipantNames(new ArrayList<String>(participantNames));
			//set all contributed participant
			setContributedPaticipantNames(dataManager.findContributedParticipantNames(getSearchCriteria().getStartDate(), new ArrayList<String>(participantNames)));
			//set all non contributed
			List<String> nonContributed = new ArrayList<String>(participantNames);
			...
			if(event!=null){
				...
				List<String> eventContributed = new ArrayList<String>(participantNames);//event contributed
		...
		try {
			return dataManager.getReportSummary(new ArrayList<String>(participantNames), getDate(), events);
		} catch (Exception e) {
			FDUtils.addMsgError("Internal Error!");
			log.error(e);
		}
